...
📧 Email : abonne@sunlib.fr`;
  ...
  const subscriber = React.useMemo(() => {
    ...
    const contractMatch = currentTicket.subscriberId.match(/SL-\d{6}/);
    if (contractMatch) {
      const contractNumber = contractMatch[0];
      console.log('🔍 Contrat extrait:', contractNumber);
      
      const foundByContract = subscribers.find(sub => sub.contratAbonne === contractNumber);
      if (foundByContract) {
    ...
    const nameMatch = currentTicket.subscriberId.match(/^(.+?)\s-\sSL-/);
    if (nameMatch) {
      const fullName = nameMatch[1].trim();
      console.log('🔍 Nom complet extrait:', fullName);
      
      const foundByName = subscribers.find(sub => {
        const subscriberFullName1 = `${sub.prenom} ${sub.nom}`;
        const subscriberFullName2 = `${sub.nom} ${sub.prenom}`;
        return subscriberFullName1.toLowerCase() === fullName.toLowerCase() ||
               subscriberFullName2.toLowerCase() === fullName.toLowerCase();
      });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-3">
                  ...
                  {/* Informations de clôture si le ticket est fermé */}
                  {currentTicket.status === 'Fermé' && (() => {
                    // Chercher le commentaire de clôture
                    const closureComment = currentTicket.comments.find(comment => 
                    ...
                    if (closureComment) {
                      ...
                      const commentMatch = closureComment.content.match(/\*\*Commentaire de clôture :\*\*\s*\n(.+?)\n\n---/s);
                      const dateMatch = closureComment.content.match(/\*Ticket fermé le (.+?)\*/);
                          ...
                          <div className="space-y-2">
                            {sourceMatch && (
                              <div className="flex items-center justify-between py-1">
                                <span className="text-sm text-gray-600">Source problématique</span>
                                <span className="text-sm font-medium text-red-700">{sourceMatch[1]}</span>
                              </div>
                            )}
                            {dateMatch && (
                              <div className="flex items-center justify-between py-1">
                                <span className="text-sm text-gray-600">Fermé le</span>
                                <span className="text-sm font-medium text-gray-900">{dateMatch[1]}</span>
                              </div>
                            )}
                            {commentMatch && (
                              <div className="py-1">
                                <span className="text-sm text-gray-600">Commentaire de clôture :</span>
                                <p className="text-sm text-gray-900 mt-1 bg-gray-100 rounded p-2">
                                  {commentMatch[1].trim()}
                                </p>
                              </div>
                            )}
        ...
        {isEditing && (
          <div className="w-80 border-l border-gray-200 bg-gray-50 p-6 overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              {showClosureForm && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 space-y-4 max-h-96 overflow-y-auto">
                  <div className="flex items-center">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <button
                      ...
                      className="px-3 py-2 text-gray-700 bg-gray-200 hover:bg-gray-300 rounded-lg transition-colors text-sm"
                    >
                      Annuler
                    </button>
                  </div>
